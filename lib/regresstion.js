  ...
  try {
    ...
    for (let run = 0; run < runs; run++) {
      ...
      const new_ad_processed = {
        ...
        text_length: new_ad.text.length,
        comments: 0
      };
    ...
    console.log('🎯 Dự đoán trung bình 5 lần cho quảng cáo mới:');
    ...
    console.log(`💬 Comments: ${Math.round(commentsSum / runs)}`);
     return {
    likes: Math.round(likesSum / runs),
    shares: Math.round(sharesSum / runs),
    comments: Math.round(commentsSum / runs)
  };
